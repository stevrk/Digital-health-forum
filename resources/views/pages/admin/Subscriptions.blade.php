        ...
        <div class="p-4">
            ...
            <div class="bg-gray-900 mb-10">
                <ul>
                    ...
                    <li class="mb-2">
                        <a href="{{route('Subscriptions')}}" class="flex items-center p-2 rounded bg-white text-sky-900 text-base md:text-lg lg:text-xl">
        ...
        <div class="bg-white shadow-md rounded-lg">
            <table id="registrations-table" class="min-w-full divide-y divide-gray-200">
                ...
                <tbody id="table-body" class="bg-gray-100 divide-y divide-gray-200">
                    ...
                    <tr data-status="{{ $subs->status }}">
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $subs->first_name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $subs->last_name }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $subs->email }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $subs->status }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            @if($subs->status !== 'approved')
                            <!-- Approve Button Form -->
                            <form action="{{route('subsapprove.approve', $subs->id) }}" method="POST" class="inline">
                                @csrf
                                @method('POST')
